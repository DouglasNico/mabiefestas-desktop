...
This typically indicates that your device does not have a healthy Internet connection at the moment. The client will operate in offline mode until it is able to successfully connect to the backend.`;
  ...
  try {
    setPersistence(auth, browserLocalPersistence).catch((err) => {
      console.warn("Aviso na persist\xEAncia do Firebase Auth:", err);
    });
  } catch (e) {
  }

  // src/js/firebase-sync.js
  var NOMES_CATEGORIAS_MAP = {
    rechaud: "R\xE9chauds",
    rechauds: "R\xE9chauds",
    toalha: "Toalhas & Mesa",
    toalhas: "Toalhas & Mesa",
    prato: "Pratos & Lou\xE7as",
    pratos: "Pratos & Lou\xE7as",
    talher: "Talheres",
    talheres: "Talheres",
    taca: "Copos & Ta\xE7as",
    tacas: "Copos & Ta\xE7as",
    ta\u00E7a: "Copos & Ta\xE7as",
    ta\u00E7as: "Copos & Ta\xE7as",
    buffet: "Buffet"
  };
  var FirebaseSync = {
    ...
    async sincronizarTudo(silencioso = false) {
      ...
      try {
        ...
        this.ultimaSincronizacao = /* @__PURE__ */ new Date();
        const horaFormatada = this.ultimaSincronizacao.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" });
        this.atualizarStatusVisual("online", `\u{1F7E2} Nuvem Conectada (${horaFormatada})`);
    ...
    iniciarOuvintesTempoReal() {
      try {
        ...
        }, (err) => console.warn("Aviso no listener de or\xE7amentos:", err));
      } catch (e) {
        console.warn("N\xE3o foi poss\xEDvel iniciar realtime listener:", e);
      }
    },
    async baixarProdutosNuvem() {
      ...
      snapshot.forEach((docSnap) => {
        ...
        const imagens = data.imagens && data.imagens.length ? data.imagens : data.imagem ? [data.imagem] : data.img ? [data.img] : [];
    ...
    async salvarProdutoNuvem(produto) {
      try {
        ...
        this.atualizarStatusVisual("online", `\u{1F7E2} Salvo na Nuvem (${hora})`);
      } catch (error) {
    ...
    processarSnapshotOrcamentos(snapshot) {
      ...
      const pedidosMap = /* @__PURE__ */ new Map();
      ...
      snapshot.forEach((docSnap) => {
        ...
        pedidosMap.set(id, {
          ...
          criadoEm: data.data || data.criadoEm || (/* @__PURE__ */ new Date()).toISOString().split("T")[0],
    ...
    async salvarPedidoNuvem(pedido) {
      try {
        ...
        console.warn("Erro ao salvar or\xE7amento no Firestore:", error);
      }
    },
    ...
    async fazerLogin() {
      ...
      try {
        ...
        if (error.code === "auth/too-many-requests") {
          ...
          mensagem = "Falha de conex\xE3o com a internet.";
        }
        this.mostrarErro(mensagem);
      } finally {
        if (btnEntrar) btnEntrar.disabled = false;
        if (btnTexto) btnTexto.textContent = "\u{1F512} Entrar no Sistema";
    ...
    async fazerLogout() {
      if (confirm("Deseja realmente sair do sistema?")) {
        try {
          ...
          this.exibirTelaLogin();
          if (window.App) {
            window.App.showToast("Sess\xE3o encerrada com sucesso.", "info");
          }
        } catch (error) {
    ...
    bindConfigForm() {
      ...
      form.addEventListener("submit", (e) => {
        ...
        config.empresa = {
          ...
          slogan: document.getElementById("cfg-slogan")?.value.trim() || "Loca\xE7\xE3o de Artigos para Festas",
          ...
          regrasLocacao: (document.getElementById("cfg-regras")?.value || "").split("\n").map((r) => r.trim()).filter((r) => r.length > 0)
        };
        StorageService.saveConfig(config);
        this.showToast("Configura\xE7\xF5es da Mabie Festas salvas com sucesso!", "success");
      });
    },
    exportarBackup() {
      ...
      a.href = url;
      a.download = `Backup_MabieFestas_${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.json`;
      a.click();
      ...
      if (window.electronAPI && window.electronAPI.onUpdaterMessage) {
        window.electronAPI.onUpdaterMessage((data) => {
          if (data.tipo === "disponivel") {
            this.showToast(`\u{1F680} Atualiza\xE7\xE3o v${data.versao} sendo baixada...`, "info");
          } else if (data.tipo === "baixado") {
            this.showToast(`\u{1F389} Vers\xE3o v${data.versao} pronta para instalar!`, "success");
            if (confirm(`Uma nova vers\xE3o (v${data.versao}) foi baixada! Deseja reiniciar o sistema agora para aplicar?`)) {
